# 前提
このアプリケーションは、Node.jsで開発され、最終的に`pkg`ツールを使用して単一の実行ファイルにパッケージ化され、配布されることを前提とします。これにより、ユーザーはNode.jsランタイムがインストールされていない環境でもアプリを直接実行できます。
...
* **配布の容易性**: ユーザーが手軽にアプリを利用開始できること。**最終的に`pkg`を使用して単一の実行ファイルとして配布されることを前提とする。**
...
* **配布ツール**: **`pkg`** (npmパッケージ)
    * **Node.jsアプリケーションを、Node.jsランタイムがインストールされていない環境でも動作する単一の実行ファイル（Windows, macOS, Linux向け）にパッケージ化することを前提とする。このための設定や構造を考慮してコードを生成すること。**
